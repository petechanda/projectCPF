class DashboardsController < ApplicationController
	...
	def edit
		...
		@not_plan = []
		for em in @employee
			emp_id_department.append(em.id)
			if Plan.find_by(:date=>@today, :user_id=>em.id) == nil
				@not_plan.append(em)
			end
		end
		@emp_plan = Plan.where(:date => @today, :user_id => emp_id_department).order(params[:sort])
		...
		if (params.key?('choose'))
			...
			for em in @employee
				emp_id_department.append(em.id)
				if Plan.find_by(:date=>@date_in, :user_id=>em.id) == nil
					@not_plan.append(em)
				end
			end
			@emp_plan = Plan.where(:date => @date_in, :user_id => emp_id_department).order(params[:sort_choose])
		end
